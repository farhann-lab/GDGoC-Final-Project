...
const Navbar = ({ searchQuery, setSearchQuery }) => {
  ...
  return (
    <nav className="bg-card/100 border-b border-border sticky top-0 z-50 backdrop-blur-3xl">
      <div className="w-full max-w-[1600px] mx-auto px-4 md:px-10 py-5 md:py-7 flex items-center gap-3 md:gap-6">
        ...
        >
          ...
          <span className="hidden md:block text-2xl lg:text-3xl font-bold tracking-tight text-foreground">
          ...
          >
            <div className={`relative transition-all duration-300 ${focused ? "scale-[1.01]" : ""}`}>
              ...
              <input
                ...
                className="w-full pl-10 md:pl-12 pr-4 py-2 md:py-2.5 bg-input-background border border-border rounded-full focus:outline-none focus:ring-2 focus:ring-primary transition-all text-sm"
              />
            </div>
          </motion.div>
        )}
        ...
        <div className={`flex gap-1 md:gap-2 items-center ${!isHome ? "ml-auto" : ""}`}>
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path}>
                <motion.div
                  className={`relative flex items-center gap-2 px-3 md:px-5 py-3 md:py-2.5 rounded-full text-sm font-medium transition-colors ${
                    isActive
